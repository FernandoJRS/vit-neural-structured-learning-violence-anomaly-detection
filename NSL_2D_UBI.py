...
adv_model.fit(generatorTrainData(batch_size_train=16),
              ...
              validation_data=generatorTestData(batch_size_train=16),
              validation_steps=int(len(test_total) / 16),
              callbacks=[tensorboard_callback, cp_callback], 
              verbose=1)
print('Training time per epoch: ' + str((time.time() - start_time_train) / 2000))
